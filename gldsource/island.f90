...
! for exact curl form, psi is required (locally called pex)
! subroutine island(ubloc,pex,erisl,isl,indj)
      include 'var.f90'

      integer i, k, lpi, ipi, jpi, isol, isl, indj

      real ubloc(2,0:maxi+1,0:maxj),erisl,cor,tv1,tv2
! real pex(0:maxi,0:maxj)
...
! for exact curl form use corex
! corex = - s(jpi)*0.25*(pex(ipi+1,jpi) + pex(ipi+1,jpi-1)
! &    - pex(ipi-1,jpi) - pex(ipi-1,jpi-1))*rh(1,ipi,jpi)
         else
            cor = sv(jpi)*0.25*(ubloc(1,ipi-1,jpi) + ubloc(1,ipi,jpi)&
             + ubloc(1,ipi-1,jpi+1) + ubloc(1,ipi,jpi+1))
! corex = - sv(jpi)*0.25*(pex(ipi,jpi+1) + pex(ipi-1,jpi+1)
! &    - pex(ipi,jpi-1) - pex(ipi-1,jpi-1))*rh(2,ipi,jpi)
         endif
! erisl = erisl + sign(1,lpi)*((drag(abs(lpi),ipi,jpi)
! &      *ubloc(abs(lpi),ipi,jpi)
         erisl = erisl + sign(1,lpi)*(drag(abs(lpi),ipi,jpi) *ubloc(abs(lpi),ipi,jpi)&
          + cor - indj*tau(abs(lpi),ipi,jpi)*rh(abs(lpi),ipi,jpi)) *(c(jpi)*dphi*&
          (2.0 - abs(lpi)) + rcv(jpi)*ds*(abs(lpi) - 1.0))
! &      + rcv(jpi)*ds*(abs(lpi) - 1.0)) + corex)
